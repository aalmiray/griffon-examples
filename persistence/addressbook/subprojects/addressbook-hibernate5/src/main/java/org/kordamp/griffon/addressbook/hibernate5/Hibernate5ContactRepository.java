...
public class Hibernate5ContactRepository implements ContactRepository {
    @Inject
    ...
    public Contact findById(@Nonnull final Long id) {
        return hibernate5Handler.withHbm5Session((sessionFactoryName, session) -> {
    ...
    private void insert(@Nonnull final Contact contact) {
        hibernate5Handler.withHbm5Session((sessionFactoryName, session) -> {
    ...
    private void update(@Nonnull final Contact contact) {
        hibernate5Handler.withHbm5Session((sessionFactoryName, session) -> {
            ContactEntity entity = session.get(ContactEntity.class, contact.getId());
    ...
    public Collection<Contact> findAll() {
        return hibernate5Handler.withHbm5Session((sessionFactoryName, session) -> (Collection<Contact>) session.createQuery("from ContactEntity").list().stream()
