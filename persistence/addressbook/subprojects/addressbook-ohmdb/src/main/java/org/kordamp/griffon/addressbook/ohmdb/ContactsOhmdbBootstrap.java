...
@Named("contacts")
public class ContactsOhmdbBootstrap implements OhmdbBootstrap {
    @Inject
    private ContactsDataProvider contactsDataProvider;

    @Override
    public void init(@Nonnull String datasourceName, @Nonnull Db db) {
        final Table<Contact> table = db.table(Contact.class);
        contactsDataProvider.getContacts().forEach(contact -> contact.setId(table.insert(contact)));
    }
